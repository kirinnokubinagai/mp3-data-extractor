...
- ✅ **YouTubeサポート**: yt-dlpを使用したYouTube動画のダウンロード・音声抽出
- ✅ **プレビュー表示**: 動画・音声のインラインプレビュー再生
- ✅ **メタデータ表示**: 動画の長さ、サイズ、解像度などの情報表示
- 🚧 **音声抽出**: 動画から音声のみをMP3で抽出（ffmpeg.wasm使用）
...
### 5. YouTubeサポートのセットアップ（オプション）

YouTube動画をダウンロード・音声抽出する場合は、yt-dlpのnative hostをインストールする必要があります。
...
インストールスクリプトが拡張機能IDを尋ねるので、chrome://extensions/ で確認した拡張機能IDを入力してください。

#### 5.3 確認

1. Chromeを再起動
2. 拡張機能を再読み込み
3. YouTubeページで拡張機能を開く
4. 動画URLが検出されれば成功
